...
var Locache = (function()
{
	...
	api.update = function(key, subkeys, thresholds, callback)
	{
		...
		callback(results, function(users)
		{
			log('Loback:', users);
			var now = Date.now();
			for (var user in users)
			{
				...
				userData.time = userData.time < 0 ? -now : now;
				cache[user] = userData;
			}
			XT.set(key, cache);
		});
...
const ERROR_CACHE_FLUSH_THRESHOLD         = 1000 * 60 * 30;           // 30m
...
const SERVER_URL                          = 'http://iggyhopper.strangled.net/gm/cdb/app/';
var VERSION                               = 'dev';
	...
	if (isSignedIn) {
		...
		if (~url.search(/user\/edit/i)) {
					...
					switch (opt.type) {
						...
						case 'list':
							nodes.push(
								$('<label/>').attr({for: 'cdb_' + opt.key, title: opt.name}).text(opt.name + ':'),
								$('<div/>').addClass('container_textbox container_textfield').css('background', 'none').append(
									$('<select/>').attr({id: 'cdb_' + opt.key, key: opt.key})
								)
							);
							break;
					}
            ...
            $('#btn_saveCoup').click(function() {
                ...
                $.post('http://iggyhopper.strangled.net/gm/cdb/app/set?ver=dev', pd, function(res) {
					uAlert(res.status === 200 ? 'Coup published successfully!' : 'Error! I don\'t know!');
				}, 'json').fail(function() {
                ...
                if (window.XULElement || window.opera) {
                    if (result[0] && result[0].ver > VERSION)
                        uAlert('There is an update for Coup d\'Bungie: ' + result[0].ver, '<br/><br/>', $('<a/>').attr('href',
                            window.XULElement ? 'http://iggyhopper.strangled.net/gm/cdb/download/coup_d_bungie.user.js' :
